...
1-second silence between entries (render_ru's own 0.5s lead-in on each clip
means the actual gap heard is ~1.5s, matching play_en_ru's convention).

Usage:
    uv run python scripts/make_ru_mp3.py -o ru_sentences.mp3
    uv run python scripts/make_ru_mp3.py 1,3,5-8 -o subset.mp3
    uv run python scripts/make_ru_mp3.py --voice irina -o ru_sentences.mp3
"""
...
def make_ru_mp3(ids: str | None, output: str, voice: str = "denis") -> None:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Render Russian sentences to a single mp3, 1s gaps between."
    )
    parser.add_argument(
    ...
    make_ru_mp3(args.ids, args.output, voice=args.voice)
